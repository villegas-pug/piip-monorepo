...
- [ ] T002 [P] [backend-specialist] Externalizar Oracle, issuer, audience y almacenamiento, establecer `/api/v1` y 100 MB, retirar credenciales y wallet versionados y registrar el gate de rotación por administrador humano en `apps/backend/business-domain/ms-piip/src/main/resources/application.yml`, `apps/backend/business-domain/ms-piip/src/main/resources/application-dev.yml`, `apps/backend/business-domain/ms-piip/src/main/resources/application-test.yml` y `apps/backend/business-domain/ms-piip/src/main/resources/oracle/wallet/`
- [ ] T003 [P] [backend-specialist] Crear pruebas ArchUnit que prohíban controlador a repositorio, entidades en contratos de servicio, repositorios entre módulos y paquetes `model`, `client` o `integration` en `apps/backend/business-domain/ms-piip/src/test/java/pe/gob/midagri/piip/architecture/ModularArchitectureTest.java`
...
- [ ] T009 [P] [backend-specialist] Alinear Resource Server para validar JWT sin derivar permisos PIIP de roles Keycloak y permitir anonimato solo en consulta pública en `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/config/SecurityConfig.java` y `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/config/JwtConfig.java`
- [ ] T010 [P] [backend-specialist] Implementar correlación, headers comunes y respuestas `application/problem+json` sin filtración de datos en `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/config/CorrelationIdFilter.java`, `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/config/ApiHeaders.java` y `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/config/ProblemDetailsConfig.java`
...
- [ ] T019 [P] [backend-specialist] Implementar `AuditService` con éxitos en la transacción de negocio, denegaciones `REQUIRES_NEW` y fallo cerrado en `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/auditoria/service/AuditService.java` y `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/auditoria/service/impl/AuditServiceImpl.java`
- [ ] T020 [P] [backend-specialist] Implementar idempotencia por consumidor, operación, clave y hash canónico en `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/auditoria/service/IdempotencyService.java` y `apps/backend/business-domain/ms-piip/src/main/java/pe/gob/midagri/piip/auditoria/service/impl/IdempotencyServiceImpl.java`
...
- [ ] T027 [frontend-specialist] Implementar shell accesible, selector de una asignación efectiva y rutas institucionales CSR frente a consulta pública SSR en `apps/frontend/src/app/core/layout/institutional-shell.component.ts`, `apps/frontend/src/app/core/effective-assignment/effective-assignment-selector.component.ts`, `apps/frontend/src/app/app.routes.ts` y `apps/frontend/src/app/app.routes.server.ts`
- [ ] T028 [P] [frontend-specialist] Crear pruebas Vitest de autenticación, interceptores, asignación efectiva, errores y navegación del shell en `apps/frontend/src/app/core/auth/auth.service.spec.ts`, `apps/frontend/src/app/core/http/auth.interceptor.spec.ts`, `apps/frontend/src/app/core/effective-assignment/effective-assignment.service.spec.ts` y `apps/frontend/src/app/core/layout/institutional-shell.component.spec.ts`
...
- [ ] T044 [US1] [database-specialist] Crear versiones independientes PEI/POI y registrar 005 y 006 inmediatamente como `PENDIENTE` en `database/ddl/organizacion/005_objetivo_pei_versionado.sql`, `database/ddl/organizacion/006_actividad_poi_versionada.sql` y `database/CHANGELOG.md`
- [ ] T045 [US1] [database-specialist] Ampliar `PROYECTO` para 23 campos, prefijo aprobado, `ADMINISTRACION` legacy nullable y mapeo de descripción bloqueante, y registrar 009 como `PENDIENTE` en `database/ddl/portafolio/009_proyecto_campos_oficiales.sql` y `database/CHANGELOG.md`
...
- [ ] T094 [P] [US7] [database-specialist] Crear pruebas SQL de publicación, clasificación, propietario y conservación tras reclasificación en `database/tests/004_documento_publicacion_test.sql`, `database/tests/013_clasificacion_campos_test.sql` y `database/tests/004_013_consulta_publica_privacidad_test.sql`
...
- [ ] T096 [P] [US7] [frontend-specialist] Tras aprobar gates, mediciones y metas de ambas consultas, crear Vitest y Playwright con allowlist, SSR, ausencia de descarga y axe en `apps/frontend/src/app/features/consulta-publica/public-detail.component.spec.ts`, `apps/frontend/src/app/features/consulta-institucional/portfolio-detail.component.spec.ts`, `apps/frontend/e2e/consulta/consulta-institucional.spec.ts`, `apps/frontend/e2e/consulta/consulta-publica.spec.ts` y `apps/frontend/e2e/accessibility/consulta.a11y.spec.ts`

### Implementation for User Story 7

- [ ] T097 [US7] [database-specialist] Crear publicación y clasificación append-only y registrar 004 y 013 inmediatamente como `PENDIENTE` en `database/ddl/documentos/004_documento_publicacion.sql`, `database/ddl/portafolio/013_clasificacion_campos.sql` y `database/CHANGELOG.md`
...
- [ ] T102 [US7] [frontend-specialist] Registrar rutas lazy y modos CSR/SSR de consulta en `apps/frontend/src/app/features/consulta-institucional/consulta-institucional.routes.ts`, `apps/frontend/src/app/features/consulta-publica/consulta-publica.routes.ts`, `apps/frontend/src/app/app.routes.ts` y `apps/frontend/src/app/app.routes.server.ts`
...
**Purpose**: Completar semillas, corte, contratos y verificaciones constitucionales sin ejecutar recursos externos automáticamente.
...
- [ ] T111 [database-specialist] Tras recibir PEI/POI y el `sub` proporcionado por el administrador Keycloak de OGTI, crear 020 y la semilla 021 manual, fail-fast y de ejecución única: prevalidar y reutilizar `MIDAGRI`/`Ministerio de Desarrollo Agrario y Riego`, insertar `ADMINISTRADOR_PIIP`/`Administrador PIIP`, combinación `GlobalAdmin`, usuario por `sub`, primera asignación y auditoría con Jefatura autorizante, aprobación de despliegue, DBA, fecha, operación y resultado; abortar sin cambios ante unidad discordante, cualquier asignación histórica o reejecución; registrar ambos scripts inmediatamente como `PENDIENTE` en `database/seeds/020_planeamiento_inicial_aprobado.sql`, `database/seeds/021_matriz_funcional_inicial_aprobada.sql`, `database/tests/020_planeamiento_inicial_aprobado_test.sql`, `database/tests/021_matriz_funcional_inicial_aprobada_test.sql` y `database/CHANGELOG.md`
...
- [ ] T118 [P] [backend-specialist] Completar pruebas MockMvc de contratos, headers, ETag, idempotencia y Problem Details en `apps/backend/business-domain/ms-piip/src/test/java/pe/gob/midagri/piip/contract/`
...
- [ ] T120 [P] [frontend-specialist] Verificar estáticamente contraste, reflow, foco, nombres accesibles, anuncios y rutas SSR/CSR en `apps/frontend/src/app/` y `apps/frontend/e2e/accessibility/`
...
2. Phase 2 bloquea todas las historias.
...
- El usuario revisa y ejecuta manualmente el incremento en orden; ningún agente ejecuta SQL.
...
- C2 se implementa solo en 021. El `database-specialist` deposita y registra el script, pero nunca lo
  ejecuta; un DBA de OGTI lo ejecuta manualmente tras revisión y aprobación.
- 021 exige 002, 007 y 008 en `EXECUTION_CONFIRMED`, `sub`, aprobación de despliegue y datos del DBA;
  aborta sin cambios ante cualquier antecedente `GlobalAdmin` o reejecución.
...
- Dentro de T044 se preparan PEI y POI de forma independiente, pero `CHANGELOG` se edita de forma
  serial; T046 espera T045 y T017.
...
| FR-026–FR-028, FR-042, FR-054–FR-066, FR-101–FR-110, FR-161 | US6 seguridad | T015–T021, T084–T093 |
...
| SC-001–SC-138 | Verificación por historia y transversal | T024, T029–T043, T053–T063, T068–T085, T094–T105, T118–T124 |
...
- No se implementan PIDE, sincronizaciones externas, controles antimalware funcionales, correo de
  remisión ni purga automática.
